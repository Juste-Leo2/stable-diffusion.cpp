...
if [ ! -d "stable-diffusion.cpp" ] && [ ! -f "build/bin/sd-cli" ]; then
    ...
    pkg install -y clang cmake git ninja vulkan-headers vulkan-loader-generic shaderc python spirv-headers wget curl
...
if [ ! -f "build/bin/sd-cli" ]; then
    echo -e "${GREEN}Building the project (Vulkan)...${NC}"
